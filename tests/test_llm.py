...
from core.llm import (
    LLMMessage, LLMResponse, LLMConfig, LLMError, LLMParseError,
    DeepSeekProvider, OllamaProvider, create_provider,
    parse_llm_json, parse_llm_json_list, with_retry,
)
...
class TestLLMConfig:
    def test_default_values(self):
        config = LLMConfig(api_key="test-key", base_url="http://localhost:11434/v1", model="llama3.1")
...
class TestDeepSeekProvider:
    ...
    def test_init(self, mock_openai):
        config = LLMConfig(api_key="test-key", base_url="https://api.deepseek.com/v1", model="deepseek-chat")
...
class TestOllamaProvider:
    ...
    def test_init_custom_config(self, mock_openai):
        config = LLMConfig(api_key="custom-key", base_url="http://custom:11434/v1", model="custom-model")
        provider = OllamaProvider(config)
        assert provider.config.api_key == "custom-key"
    @patch('core.llm.OpenAI')
    def test_init_from_env(self, mock_openai):
        with patch.dict(os.environ, {'OLLAMA_BASE_URL': 'http://env-host:11434/v1', 'OLLAMA_MODEL': 'env-model'}):
            provider = OllamaProvider()
