...
REDBLINE_VIOLATIONS = [
    ...
    "正文中出现写作规划元信息（如'写前蓝图''核心冲突''情感旅程''必须推进的伏笔''结尾钩子''节拍序列''节奏建议''登场角色'等，这些是给写手的指令，不属于小说内容）",
    "报告式语言出现（分析了形势/综合考虑等）",
    "集体反应套话出现（全场震惊/众人哗然等）",
...
## 特殊规则：配套文档缺失
- 写后结算表为空（或全部为默认值）→ 记为 info，不计入 weighted_total
- 真相文件"尚未更新"或为空 → 同理记为 info，不计入 weighted_total
- 例外：如果正文确实存在连续性错误（如角色位置矛盾、情感前后不一致），则直接以「连续性」维度报 critical，而不是报「结算表为空」
- 简单说：空文档 ≠ 内容差。第一章尤其如此（没有前文可供连续性验证）
...
## 🔴 特别检查项（必须优先执行）
**检查正文中是否混入了写作规划元信息**：扫描全文，如果发现以下任何内容出现在正文中，必须标记为 critical + 红线违规：
- "写前蓝图"、"写作蓝图"
- "**核心冲突**"、"核心冲突："
- "**情感旅程**"、"情感旅程："
- "**必须推进的伏笔**"、"计划埋下伏笔"
...
- "**节拍序列**"、"节奏建议"
- "**登场角色**"（以列表形式出现，非对话中提及）
- 任何类似"* **XX：** 描述"的规划格式（星号加粗标题+冒号+描述）

这些是写手指令，不是小说内容。一旦出现，无论多少，直接触发红线第6条。
